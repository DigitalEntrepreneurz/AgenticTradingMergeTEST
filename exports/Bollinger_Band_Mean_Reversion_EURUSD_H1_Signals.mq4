//+------------------------------------------------------------------+
//| Bollinger_Band_Mean_Reversion_EURUSD_H1_Signals.mq4  --  SIGNAL INDICATOR (does not trade)
...
int DecideAt(int sh, double atr)
  {
   ...
   {
    double up=iBands(NULL,0,20,2.0,0,PRICE_CLOSE,MODE_UPPER,sh), lo=iBands(NULL,0,20,2.0,0,PRICE_CLOSE,MODE_LOWER,sh);
    double px=Cl(sh);
    if(up<=0||lo<=0)   ev[0]=0;
   ...
   {
    double r=iRSI(NULL,0,14,PRICE_CLOSE,sh);
    if(r<=0)        ev[1]=0;
...
int OnInit()
  {
   ...
   IndicatorShortName("Bollinger_Band_Mean_Reversion_EURUSD_H1_Signals");
   return INIT_SUCCEEDED;
  }

void OnDeinit(const int reason) { ObjectDelete("Bollinger_Band_Mean_Reversion_EURUSD_H1_Signals_panel"); }
   ...
   if(AlertOnSignal && dirNow!=0)
     {
      if(Time[0]!=lastAlert)
        {
         lastAlert=Time[0];
         Alert("Bollinger_Band_Mean_Reversion_EURUSD_H1_Signals: ",(dirNow>0?"BUY":"SELL")," setup on ",Symbol());
        }
     }
   return rates_total;
  }
