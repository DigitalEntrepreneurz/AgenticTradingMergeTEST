...
int DecideAt(int sh, double atr)
  {
   ...
   {
    double f=iMA(NULL,0,9,0,MODE_EMA,PRICE_CLOSE,sh), m=iMA(NULL,0,21,0,MODE_EMA,PRICE_CLOSE,sh), s=iMA(NULL,0,50,0,MODE_EMA,PRICE_CLOSE,sh);
   ...
   {
    double ma=iMA(NULL,0,21,0,MODE_EMA,PRICE_CLOSE,sh), px=Cl(sh);
...
int OnInit()
  {
   ...
   IndicatorShortName("T_Both_Signals");
   return INIT_SUCCEEDED;
  }

void OnDeinit(const int reason) { ObjectDelete("T_Both_Signals_panel"); }
   ...
   if(AlertOnSignal && dirNow!=0)
     {
      if(Time[0]!=lastAlert)
        {
         lastAlert=Time[0];
         Alert("T_Both_Signals: ",(dirNow>0?"BUY":"SELL")," setup on ",Symbol());
        }
     }
   return rates_total;
  }
