//+------------------------------------------------------------------+
//| T_IndOnly_Signals.mq5  --  SIGNAL INDICATOR (does not trade)
...
void Panel(int dir, double atr)
  {
   if(!ShowPanel) return;
   string nm="T_IndOnly_Signals_panel";
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"T_IndOnly_Signals");
   return INIT_SUCCEEDED;
  }

void OnDeinit(const int reason) { ObjectDelete(0,"T_IndOnly_Signals_panel"); }
   ...
   if(AlertOnSignal && dirNow!=0)
     {
      ...
      if(bt!=lastAlert)
        {
         lastAlert=bt;
         Alert("T_IndOnly_Signals: ",(dirNow>0?"BUY":"SELL")," setup on ",_Symbol," ",EnumToString(_Period));
        }
     }
   return rates_total;
  }
